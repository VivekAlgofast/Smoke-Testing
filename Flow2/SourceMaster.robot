...
Source master Form Keyword
    ...
    Sleep   1s
    Click Element    xpath=//span[normalize-space()='Source of Enquiry']
    Sleep    3s
    Click Button    xpath=//button[normalize-space()='Add New']
    Wait Until Element Is Visible    xpath=//input[@id='source_master']
    Input Text    xpath=//input[@id='source_master']  ${Source_master_value}
    Click Button    xpath=//button[normalize-space()='Submit']
    Sleep   3s
     ${name_entered}    Get Value    css=#source_master
    ...
    ${sql_query}=    Set Variable    SELECT name FROM enquiry_source_master WHERE name='${name_entered}' LIMIT 1;
