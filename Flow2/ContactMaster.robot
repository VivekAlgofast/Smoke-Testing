...
contact master Form Keyword
    ...
    Mouse over     xpath=//span[normalize-space()='Contact Masters']
    Click Element    xpath=//span[normalize-space()='Contacts']
    Sleep   3s
    Click Button    xpath=//button[normalize-space()='Add New']
    Wait Until Element Is Visible    xpath=//input[@id='name']
    Input Text    xpath=//input[@id='name']  ${contact_master_value}
    Click Element    xpath=//input[@id='contactGroup']
    Sleep    3s
    Click Element    xpath=//li[@id='contactGroup-option-0']
    ...
    ${sql_query}=    Set Variable    SELECT name FROM contact_master WHERE name='${name_entered}' LIMIT 1;
